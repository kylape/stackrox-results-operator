...
make install
make deploy
k -n stackrox-results-operator-system  set image deploy/stackrox-results-operator-controller-manager '*=kind-registry:5000/stackrox-results-operator:latest'
